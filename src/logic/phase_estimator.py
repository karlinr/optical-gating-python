...
from logic.utils import v_fitting, sad_with_references 
from app.config import Config

TWO_PI = 2 * np.pi
NUM_EXTRA_REF_FRAMES = 2 # numExtraRefFrames from your snippet
...
class SADEstimator(PhaseEstimator):
    ...
    def __init__(self):
        super().__init__()
        self.reference_frames = None    # self.ref_frames
        self.reference_period = None    # self.ref_period
        self.barrier_phase = None       # self.targetSyncPhase
        self.frame_history = []         # List of (frame, timestamp)
        self.period_history = []        # For stability checks
    ...
    def add_sample(self, frame, **kwargs):
        ...
        # Store as tuple to keep metadata attached
        self.frame_history.append((frame, timestamp))

        # Trim buffer based on minimum heart rate
        ...
        # Check if we can establish a period yet
    ...
    def estimate(self, frame):
        ...
        # Offset calculation similar to find_subframe_minimum in your snippet
